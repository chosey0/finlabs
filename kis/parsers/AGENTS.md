...
## Purpose
Converts KIS REST responses (JSON dicts) or WebSocket frames (pipe-delimited strings) into frozen dataclass instances from `kis.models`. All type coercion — Decimal/int conversion, date/time parsing, and sign normalization — is concentrated here, keeping model code clean and data-only.
...
- To add a new endpoint: (1) add a `parse_<...>` function to `rest.py` → (2) add to `__init__.py`'s `__all__` → (3) export from the package surface `kis/__init__.py`.
...
### Internal
- `kis.models.*` — target dataclasses
- `kis.exceptions` — `KisRealtimeError` (on realtime parse failure)
