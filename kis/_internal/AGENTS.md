...
## Purpose
Internal SDK transport and header builders. As the underscore prefix implies, this is **not a public surface** — do not import from outside the `kis` package. External users reach this layer only via `kis.KisClient.request()`.
...
- Header builders accept only frozen `Credentials`. Token issuance logic lives in `kis.auth.oauth`.
