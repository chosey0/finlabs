...
### `collect_rss_loop.sh`
`collect-rss`를 고정 간격(기본 60초)으로 반복 실행합니다. 멱등(`ON CONFLICT DO NOTHING`)이라 매번 새 항목만 적재하고 중복은 건너뜁니다. 자기 자신과 절대 중첩하지 않고(실행 → 다음 경계까지 sleep), 원자적 `mkdir` 잠금으로 단일 인스턴스를 보장하며, `SIGINT`/`SIGTERM`에 종료합니다. 추가 인자는 `collect-rss`로 전달됩니다.
