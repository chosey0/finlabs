...
def train(features: Sequence[FeatureVector], *, config: TrainConfig) -> TrainResult:
    ...
    for _ in range(config.epochs):
        ...
        for start in range(0, inputs.size(0), config.batch_size):
            ...
            loss = reconstruction_loss + codebook_loss + config.model.commitment_cost * commitment_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    return TrainResult(checkpoint_path=checkpoint_path, epochs=config.epochs, final_loss=final_loss)
