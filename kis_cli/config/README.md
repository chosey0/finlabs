...
`kis_cli.config`는 FinLabs KIS CLI가 사용하는 로컬 설정, 프로필, 시크릿 참조 해석을 담당합니다. 설정 파일에는 시크릿 값을 직접 저장하지 않고 환경변수 참조만 저장합니다.
...
`init.py`는 `python -m kis_cli config init`에서 사용하는 설정 템플릿을 만듭니다.
...
python -m kis_cli config validate --path ./config.yaml
```
