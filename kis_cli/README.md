...
`kis_cli`는 Korea Investment & Securities Open API 기반의 시장 데이터 수집 CLI 패키지입니다. CLI 명령은 `kiscli`로 제공되며, 현재 구현 범위는 설정 관리, 인증 확인, 심볼 마스터 다운로드/검색, 현재가 조회, OHLCV 이력 수집/저장, 저장 데이터 조회/내보내기, SQLite DB 점검입니다.
...
- `storage/`: SQLite 연결, 스키마 생성, 중복 방지 insert/upsert, 조회, DB 점검
...
- SQLite DB: `~/.local/share/kis-cli/kis-cli.db`
