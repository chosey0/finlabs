...
| `test_config_init.py` | `python -m kis_cli config init`, 프로필 추가/수정/삭제 |
| `test_kis_package.py` | `kis` SDK surface 검증 — `KisClient`, 모델, 파서, EndpointSpec |
| `test_logs.py` | `python -m kis_cli logs runs/api` — SQLite `app.db` 조회 + 필터 |
| `test_price.py` | 현재가 조회 service 함수 (`get_current_price`) |
| `test_query.py` | `python -m kis_cli query ohlcv/minutes` — DuckDB 조회 + CSV/JSON 출력 |
...
- DB 테스트는 `tmp_path` fixture로 격리합니다 — 사용자 데이터 디렉토리(현재 호환 경로 `~/.local/share/kis-cli/`)를 절대 건드리지 마세요.
...
- 실행: `uv run python -m pytest` (또는 `pytest`)
...
### Internal
- `kis` — SDK 본체
- `kis_cli` — 현재 FinLabs KIS CLI + 저장소
