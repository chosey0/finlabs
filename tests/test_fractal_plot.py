...
def test_latest_fractal_event_segments_returns_latest_transition_segments():
    ...
    segments = latest_fractal_event_segments(candles, events=events, min_segment_change_pct=0)
    ...
    assert [event.index for event in segments["low_to_high"].events] == [0, 2]
...
def test_fractal_event_segments_returns_all_transition_segments_chronologically():
    ...
    segments = fractal_event_segments(candles, events=events, min_segment_change_pct=0)
...
def test_fractal_event_segments_skips_segments_with_large_time_gap():
    ...
    segments = fractal_event_segments(candles, events=events, max_gap_seconds=5 * 60, min_segment_change_pct=0)
...
def test_plot_latest_fractal_event_segments_returns_one_figure_per_transition():
    ...
    figures = plot_latest_fractal_event_segments(candles, events=events, min_segment_change_pct=0)
...
def test_plot_fractal_event_segments_returns_one_figure_per_segment():
    ...
    segment_figures = plot_fractal_event_segments(candles, events=events, min_segment_change_pct=0)
